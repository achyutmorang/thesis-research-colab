...
def run_quick_surprise_probe(
                ...
                for repeat_id in range(repeat_seeds):
                    ...
                    if (not bool(base_rollouts[repeat_id]['base_feasible'])) or (not bool(p_feasible)):
                        ...
                        p_surprise, p_score_diag = compute_counterfactual_surprise_score(
                            ...
                            belief_source_mode=str(getattr(cfg, 'surprise_belief_source_mode', 'auto')),
                    ...
                    rows.append({
                        ...
                        'surprise_belief_mode': str(p_score_diag.get('surprise_belief_mode', 'auto')) if isinstance(p_score_diag, dict) else 'auto',
    ...
    if probe_df.empty:
        summary_df = pd.DataFrame([{
            ...
            'surprise_policy_shift_raw_mean': np.nan,
            'surprise_realization_ratio_mean': np.nan,
            'surprise_realization_ratio_raw_mean': np.nan,
            'surprise_signal_floor_mean': np.nan,
            'surprise_response_floor_mean': np.nan,
    ...
    summary_df = pd.DataFrame([{
        ...
        'surprise_policy_shift_raw_mean': _safe_col_nanmean(probe_df, 'surprise_policy_shift_raw'),
        'surprise_realization_ratio_mean': _safe_col_nanmean(probe_df, 'surprise_realization_ratio'),
        'surprise_realization_ratio_raw_mean': _safe_col_nanmean(probe_df, 'surprise_realization_ratio_raw'),
        'surprise_signal_floor_mean': _safe_col_nanmean(probe_df, 'surprise_signal_floor'),
        'surprise_response_floor_mean': _safe_col_nanmean(probe_df, 'surprise_response_floor'),
